...
func main() {
	...
	// Correlate rspamd verdicts (if rspamd.log* is present in the mount) onto the
	// mail records just parsed. PMG's pmg-smtp-filter is already correlated inline
	// from mail.log; this adds the rspamd-host case.
	parseAllRspamd(*logDir, store)
	rspamdOffset := int64(0)
	...
	if rspamdLog != "" {
		go NewRspamdWatcher(rspamdLog, store, rspamdOffset).Run(ctx)
	}
